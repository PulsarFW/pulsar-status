		...
		while plsr.State.flags.loggedIn do
			if not plsr.State.flags.isDead then
						...
						if mvUpdate then
							...
							if newMovement ~= drunkMovement then
								...
								if newMovement > 0 and _drunkMovementClipsets[newMovement] then
									SetPedMovementClipset(PlayerPedId(), _drunkMovementClipsets[newMovement], 1.0)
									plsr.State.flags.drunkMovement = true
								else
						...
						if lastBlackout <= 0 then
							...
							if lastBlackout <= GetGameTimer() then
								...
								if veh and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
									TaskVehicleTempAction(PlayerPedId(), veh, _drunkVehicleActions[math.random(#_drunkVehicleActions)], 5000)
								end
				...
				elseif wasDrunk then
					wasDrunk = false
					plsr.State:SetPublicClientFlag('isDrunk', false)
					plsr.State.flags.drunkMovement = false
...
function IsDrinkingAlcohol()
    local doingAnim = plsr.Animations.Emotes:Get()
...
function RegisterDrunkCallbacks()
    plsr.Callbacks:RegisterClientCallback('Status:DrinkAlcohol', function(data, cb)
        if IsDrinkingAlcohol() then
            plsr.Notification:Error('Already Drinking!')
            return cb(false)
        end

        local alcohol = _alcoholConfig[data]
        if alcohol then
            plsr.Animations.Emotes:Play(alcohol.anim, false, alcohol.initialTime, true)
            plsr.Progress:Progress({
                ...
                if not cancelled then
                    ...
                    plsr.Animations.Emotes:Play(alcohol.anim, false, false, false)

                    local drunkLevel = plsr.Status.Get:Single("PLAYER_DRUNK").value
                    if drunkLevel < alcohol.drunkBaseline then
                        plsr.Status.Set:Single("PLAYER_DRUNK", alcohol.drunkBaseline)
                    end

                    plsr.Status.Modify:Add("PLAYER_THIRST", alcohol.thirst)
                    plsr.Status.Modify:Remove("PLAYER_STRESS", alcohol.stressRelief, true)

                    local endTime = GetGameTimer() + alcohol.maxTime

                    CreateThread(function()
                        local tick = 3
                        while plsr.State.flags.loggedIn and plsr.Animations.Emotes:Get() == alcohol.anim do
                            if tick >= 5 then
                                plsr.Status.Modify:Add("PLAYER_DRUNK", alcohol.drunkPerTick)
